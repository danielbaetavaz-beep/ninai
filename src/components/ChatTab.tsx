...
export default function ChatTab({ plan }: { plan: any }) {
  ...
  async function send() {
    ...
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({ messages: newMsgs.slice(-10), mode: 'general', planContext: { goals: plan.goals, meal_plan_base: plan.meal_plan_base, exercise_plan_base: plan.exercise_plan_base } }),
    });
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-120px)]">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2">
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${m.role === 'assistant' ? 'bg-teal-50 text-teal-900 rounded-tl-sm rounded-tr-2xl rounded-br-2xl rounded-bl-2xl' : 'bg-gray-100 text-gray-800 rounded-tl-2xl rounded-tr-sm rounded-br-2xl rounded-bl-2xl'}`}>
              {m.content}
            </div>
          </div>
        ))}
      ...
      <ExpandingInput value={input} onChange={setInput} onSend={send} disabled={loading} placeholder="Pergunte qualquer coisa..." />
    </div>
  );
}
