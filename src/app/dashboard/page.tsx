...
export default function Dashboard() {
  ...
  async function loadData() {
    ...
    if (currentPlan.status === 'onboarding') {
      window.location.href = `/onboarding?plan=${currentPlan.id}`;
      return;
    }
  ...
  if (!plan) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center">
        <h1 className="text-2xl font-bold mb-2">nin<span className="text-teal-400">AI</span></h1>
        ...
        <button
          ...
          className="px-8 py-4 bg-teal-400 text-white rounded-2xl text-base font-medium"
        >
          Criar novo plano
        </button>
      </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center">
        ...
        <p className="text-gray-700 font-medium mb-2">
          {plan.status === 'consultation_requested' ? 'A Nina solicitou uma consulta' : 'Plano aguardando revisão da Nina'}
        </p>
        <p className="text-gray-400 text-sm max-w-xs">
          {plan.status === 'consultation_requested'
            ? 'A Nina quer te conhecer pessoalmente antes de aprovar o plano. Ela vai entrar em contato para agendar.'
            : 'Seu plano foi enviado para a Nina analisar. Você será notificado quando for aprovado.'}
        </p>
      </div>
    );
  }
  ...
  if (plan.status === 'approved' && !weeklyPlan) {
    return <WeeklyRoutineSetup plan={plan} onComplete={() => loadData()} />;
  }
...
function WeeklyRoutineSetup({ plan, onComplete }: { plan: any; onComplete: () => void }) {
  ...
  const mealsPerDay = plan.meal_plan_base?.meals_per_day || 5;
  const mealNames = ['Café da manhã', 'Lanche da manhã', 'Almoço', 'Lanche da tarde', 'Jantar'].slice(0, mealsPerDay);
  ...
  function toggleMeal(day: string, meal: string) {
    setRoutine((prev: any) => {
      ...
      const next = current === 'casa' ? 'fora' : 'casa';
      return { ...prev, [day]: { ...prev[day], meals: { ...prev[day].meals, [meal]: next } } };
    });
  }

  function toggleGym(day: string) {
    setRoutine((prev: any) => ({ ...prev, [day]: { ...prev[day], gym: !prev[day].gym } }));
  }
  ...
  async function generatePlan() {
    ...
    await supabase.from('weekly_plans').insert({
      ...
      routine,
      meal_plan_detailed: weekPlan.meal_plan || {},
      exercise_plan_detailed: weekPlan.exercise_plan || {},
  ...
  return (
    <div className="p-4">
      ...
      <p className="text-gray-400 text-sm mb-4">Informe onde você vai estar em cada refeição e se terá acesso à academia.</p>
        ...
        <div key={day} className="mb-4 bg-gray-50 rounded-xl p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <label className="flex items-center gap-2 text-xs text-gray-500">
              <input type="checkbox" checked={routine[day].gym} onChange={() => toggleGym(day)} className="rounded" />
              Academia
            </label>
          </div>
          <div className="space-y-1">
            {mealNames.map(meal => (
              <div key={meal} className="flex items-center justify-between">
                ...
                <button onClick={() => toggleMeal(day, meal)} className={`text-xs px-3 py-1 rounded-full ${
                  routine[day].meals[meal] === 'casa' ? 'bg-green-50 text-green-800' : 'bg-amber-50 text-amber-800'
                }`}>
